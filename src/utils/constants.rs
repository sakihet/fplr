pub const WIDTH_ID: usize = 4;
pub const WIDTH_NAME: usize = 16;
pub const WIDTH_TEAM_NAME: usize = 14;
pub const WIDTH_POS: usize = 3;
pub const WIDTH_TEAM_SHORT_NAME: usize = 4;
pub const WIDTH_COST: usize = 4;
pub const WIDTH_SEL: usize = 4;
pub const WIDTH_FORM: usize = 4;
pub const WIDTH_PTS: usize = 3;
pub const WIDTH_POINTS: usize = 6;
pub const WIDTH_AVAIL: usize = 5;
pub const WIDTH_STAT: usize = 6;
pub const WIDTH_STAT_SMALL: usize = 4;
pub const WIDTH_TRANS: usize = 7;
pub const WIDTH_STR: usize = 3;
pub const WIDTH_TIME: usize = 20;
pub const WIDTH_SCORE: usize = 7;
pub const WIDTH_NEWS: usize = 30;
pub const WIDTH_STATUS: usize = 13;
pub const WIDTH_GW: usize = 2;
pub const WIDTH_AVERAGE: usize = 7;
pub const WIDTH_HIGHEST: usize = 7;
pub const WIDTH_RANK: usize = 4;
pub const WIDTH_RANK_WIDE: usize = 10;
pub const WIDTH_PLAYED: usize = 4;
pub const WIDTH_WIN: usize = 4;
pub const WIDTH_DRAW: usize = 4;
pub const WIDTH_LOSS: usize = 4;
pub const WIDTH_GD: usize = 5;
pub const WIDTH_SEASON: usize = 7;
pub const WIDTH_COST_RANGE: usize = 9;
pub const WIDTH_MINUTES: usize = 5;
pub const WIDTH_DATE: usize = 24;
pub const WIDTH_FULL_NAME: usize = 20;
pub const WIDTH_HA: usize = 5;
pub const WIDTH_AVG: usize = 5;
pub const WIDTH_DIFFICULTY: usize = 15;
pub const WIDTH_FDR_STAT: usize = 10;
pub const WIDTH_FORM_DIFF: usize = 12;
pub const WIDTH_STAT_WIDE: usize = 8;
pub const WIDTH_FDR_CELL: usize = 16;
pub const WIDTH_LONG_NAME: usize = 25;
pub const WIDTH_ENTRY_ID: usize = 12;
pub const WIDTH_ABBR: usize = 6;
pub const WIDTH_ISO: usize = 4;
